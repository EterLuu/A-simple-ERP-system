...
public class ReceiptsTableModel extends AbstractTableModel implements Serializable {

    private static final long serialVersionUID = -3509664072552202155L;
    private String[] columnNames = {"ID", "Customer", "Incoming", "Time", "Remark"};
    private final List<Object[]> data = new ArrayList<>();

    public ReceiptsTableModel(List<ReceiptsEntity> results) {

        CustomerServiceImpl customerService = new CustomerServiceImpl();

        for (ReceiptsEntity order : results) {
            Object[] obj = new Object[columnNames.length];
            obj[0] = order.getRid();
            obj[1] = customerService.selectCustomer(order.getCid()).getName();
            obj[2] = order.getBalance() + Context.CURRENCYSYMBOL;
